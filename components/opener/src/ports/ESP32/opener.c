...
static SemaphoreHandle_t opener_init_mutex_creation_mutex = NULL;
static bool opener_initialized = false;
TaskHandle_t opener_task_handle = NULL;
volatile int g_end_stack = 0;

static SemaphoreHandle_t get_opener_init_mutex(void) {
  if (opener_init_mutex != NULL) {
    return opener_init_mutex;
  }
  
  if (opener_init_mutex_creation_mutex == NULL) {
    opener_init_mutex_creation_mutex = xSemaphoreCreateMutex();
    if (opener_init_mutex_creation_mutex == NULL) {
      return NULL;
    }
  }
  
  if (xSemaphoreTake(opener_init_mutex_creation_mutex, portMAX_DELAY) == pdTRUE) {
    if (opener_init_mutex == NULL) {
      opener_init_mutex = xSemaphoreCreateMutex();
    }
    xSemaphoreGive(opener_init_mutex_creation_mutex);
  }
...
void opener_init(struct netif *netif) {
  opener_init_mutex = get_opener_init_mutex();
  if (opener_init_mutex == NULL) {
  ...
  if (IfaceLinkIsUp(netif)) {
    ...
    // NOTE: g_tcpip access here is safe because:
    // 1. This runs during initialization before OpENer task starts
    // 2. WebUI is initialized after OpENer, so no concurrent access yet
    // 3. Once OpENer task starts, g_tcpip is only modified via NvTcpipStore()
    //    which should be protected by mutex if called from WebUI
    
    // Try to load from system_config first (same source as main.c uses)
    system_ip_config_t sys_config;
    bool loaded_from_system_config = system_ip_config_load(&sys_config);
    
    if (loaded_from_system_config) {
      // Convert system_config to OpENer's g_tcpip structure
      if (sys_config.use_dhcp) {
      ...
      g_tcpip.interface_configuration.ip_address = sys_config.ip_address;
      g_tcpip.interface_configuration.network_mask = sys_config.netmask;
      g_tcpip.interface_configuration.gateway = sys_config.gateway;
      g_tcpip.interface_configuration.name_server = sys_config.dns1;
      g_tcpip.interface_configuration.name_server_2 = sys_config.dns2;
      OPENER_TRACE_INFO("Loaded TCP/IP configuration from system_config\n");
    } else {
      // Fall back to OpENer's own NVS
      EipStatus nv_status = NvTcpipLoad(&g_tcpip);
      if (kEipStatusOk == nv_status) {
        OPENER_TRACE_INFO("Loaded TCP/IP configuration from OpENer NVS\n");
      } else {
        OPENER_TRACE_INFO("No saved TCP/IP configuration in NVS, using interface defaults\n");
      }
    }
