...
//    }
    ...
    if(socket_timer == NULL) {
      OPENER_TRACE_WARN("networkhandler: no available session slot, closing new connection on socket %d\n", new_socket);
      ShutdownSocketPlatform(new_socket);
      CloseSocketPlatform(new_socket);
      return;
    }
